...
public class OrderCrossOver extends CrossOver
{
	...
	public int[] apply1(Represent.Individual p1, Represent.Individual p2)
	{
		int n=p1.getPermutation().length;
		...
		{
			for (int ii=i;ii<=j;ii++)
				newperm[ii]=p1.getPermutation()[ii];

			int t=j+1;
			for (int ii=j+1;ii<j+1+n;ii++)
			{
				if (ismember(newperm,p2.getPermutation()[ii%n])<0)
				{
					newperm[t%n]=p2.getPermutation()[ii%n];
					t++;
				}
			}
		}
		else
		{
			...
			for (int ii=j-1;ii>j-1-n;ii--)
			{
				...
				if (ismember(newperm,p2.getPermutation()[mii%n])<0)
				{
					if (t<0)
						t=t+n;

					newperm[t%n]=p2.getPermutation()[mii%n];
					t--;
				}
			}
		}
